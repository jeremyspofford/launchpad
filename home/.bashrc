...
# HISTSIZE and HISTFILESIZE are now in .commonrc
# HISTSIZE=1000
# FILESIZE=2000
...
# mise activation - only if mise is installed
if command -v mise &>/dev/null; then
    eval "$(mise activate bash)"
fi

# Starship prompt (if installed) - STARSHIP_CONFIG is in .commonrc
if command -v starship &>/dev/null; then
  eval "$(starship init bash)"
fi

# NVM_DIR is now in .commonrc
...
if grep -qi microsoft /proc/version 2>/dev/null; then
    ...
    # Use Windows home directory for some operations
    ...
    # Fix for slow git in WSL when accessing Windows drives
...
if [[ -o interactive ]]; then
    if [[ -n "$IS_WSL" ]]; then
        echo "Welcome to WSL on $(hostname), $(whoami)!"
    else
